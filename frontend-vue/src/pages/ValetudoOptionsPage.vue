...
import {fetchUpdaterConfiguration, fetchValetudoCustomizations, sendDuststreamingConfiguration, sendRestoreDefaultConfigurationAction, sendUpdaterConfiguration, sendValetudoCustomizations} from "../api/client";
...
<template>
    <div class="page max-w-3xl">
        <PageHeader :title="$t('Valetudo options')" :subtitle="$t('Tunables and actions provided by Valetudo')" />
        <section class="panel">
            <div class="space-y-5">
                ...
                <label class="flex items-center gap-3 border-b pb-4" style="border-color: var(--app-border)"><Checkbox :model-value="updater.data.value?.enabled ?? false" binary :disabled="updater.isPending.value || updater.isError.value || saveUpdater.isPending.value" @update:model-value="value => saveUpdater.mutate({enabled: Boolean(value)})" /> {{ $t("Enable updater") }}</label>
