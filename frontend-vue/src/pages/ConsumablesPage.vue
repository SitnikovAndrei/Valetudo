...
function remainingLabel(consumable: ConsumableMeta): string {
    ...
    if (!state) return "";
    return state.unit === "minutes" ? formatMinutes(state.value, locale.value) : `${state.value} ${valueLabel(state.unit)}`;
}
...
<template>
    <div class="page">
        <PageHeader :title="$t('Consumables')" :subtitle="$t('Monitor and reset consumable states')" />
        <section class="panel">
