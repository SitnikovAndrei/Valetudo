...
<template>
    <section class="grid gap-5 md:grid-cols-2">
        <h1 class="text-2xl font-bold md:col-span-2">Robot system options</h1>
        <div v-if="speakerEnabled" class="panel"><h2 class="mb-4 text-xl font-semibold">Speaker</h2><p v-if="speaker.isPending.value" role="status">Loading…</p><template v-else><label class="flex flex-col gap-2">Volume <InputNumber v-model="volume" :min="0" :max="100" :disabled="speaker.isError.value || saveVolume.isPending.value" :use-grouping="false" /></label><div class="mt-4 flex gap-2"><Button label="Apply volume" :disabled="volume === speaker.data.value?.volume" :loading="saveVolume.isPending.value" @click="saveVolume.mutate(volume)" /><Button label="Test sound" outlined :loading="test.isPending.value" @click="test.mutate()" /></div></template><Message v-if="speaker.isError.value || saveVolume.isError.value || test.isError.value" severity="error" class="mt-4">Speaker request failed.</Message></div>
        <div v-if="capabilities.includes(Capability.DoNotDisturb)" class="panel"><h2 class="mb-4 text-xl font-semibold">Do not disturb</h2><p v-if="dnd.isPending.value" role="status">Loading…</p><template v-else><label class="flex items-center gap-2"><Checkbox v-model="draft.enabled" binary /> Enabled</label><div class="mt-4 flex gap-4"><label class="flex flex-col gap-1">Start <input v-model="start" type="time" :disabled="!draft.enabled" class="rounded-lg border p-2" style="background: var(--app-surface); border-color: var(--app-border)" /></label><label class="flex flex-col gap-1">End <input v-model="end" type="time" :disabled="!draft.enabled" class="rounded-lg border p-2" style="background: var(--app-surface); border-color: var(--app-border)" /></label></div><Button label="Apply" class="mt-4" :loading="saveDnd.isPending.value" @click="saveDndConfiguration" /></template><Message v-if="dnd.isError.value || saveDnd.isError.value" severity="error" class="mt-4">Do not disturb request failed.</Message></div>
        <div v-if="capabilities.includes(Capability.VoicePackManagement)" class="panel md:col-span-2"><h2 class="mb-4 text-xl font-semibold">Voice packs</h2><p v-if="voice.isPending.value" role="status">Loading…</p><template v-else><p>Current language: {{ voice.data.value?.currentLanguage }}</p><p class="mt-2">{{ voice.data.value?.operationStatus.type }}</p><ProgressBar v-if="['downloading', 'installing'].includes(voice.data.value?.operationStatus.type ?? '')" class="my-4" :value="voice.data.value?.operationStatus.progress" :mode="voice.data.value?.operationStatus.progress === undefined ? 'indeterminate' : 'determinate'" /><form class="mt-4 grid gap-3" @submit.prevent="saveVoice.mutate({action: 'download', url, language, hash})"><label class="flex flex-col gap-1">URL <InputText v-model="url" type="url" placeholder="https://" required /></label><label class="flex flex-col gap-1">Language code <InputText v-model="language" required /></label><label class="flex flex-col gap-1">Hash <InputText v-model="hash" required /></label><Button type="submit" label="Set voice pack" :disabled="['downloading', 'installing'].includes(voice.data.value?.operationStatus.type ?? '')" :loading="saveVoice.isPending.value" /></form></template><Message v-if="voice.isError.value || saveVoice.isError.value" severity="error" class="mt-4">Voice pack request failed.</Message></div>
    </section>
</template>
