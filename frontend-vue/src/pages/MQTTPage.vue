...
<template>
    <div class="page max-w-4xl">
        <PageHeader :title="$t('MQTT connectivity')">
            <template #actions><Button :label='$t("Refresh status")' text :loading="status.isFetching.value" @click="status.refetch()" /></template>
        </PageHeader>
        <section class="panel">
            ...
            <form v-else-if="draft" class="grid gap-5" @submit.prevent="submit">
                ...
                <fieldset class="grid gap-3 border-t pt-4" style="border-color: var(--app-border)"><legend class="font-semibold">{{ $t("Connection") }}</legend>
                    <label class="flex flex-col gap-1">{{ $t("Broker host") }} <InputText v-model="draft.connection.host" required /><span class="muted text-sm">{{ $t("The MQTT Broker hostname") }}</span></label>
                    <label class="flex flex-col gap-1">{{ $t("Port") }} <InputNumber v-model="draft.connection.port" :min="1" :max="65535" :use-grouping="false" required /><span class="muted text-sm">{{ $t("The MQTT Broker port") }}</span></label>
                    <label class="flex items-center gap-2"><Checkbox v-model="draft.connection.tls.enabled" binary /> {{ $t("TLS") }}</label>
                    <template v-if="draft.connection.tls.enabled"><label class="flex flex-col gap-1">{{ $t("Certificate authority") }} <Textarea v-model="draft.connection.tls.ca" rows="3" /><span class="muted text-sm">{{ $t("The optional Certificate Authority to verify the connection with") }}</span></label><label class="flex items-center gap-2"><Checkbox v-model="draft.connection.tls.ignoreCertificateErrors" binary /> {{ $t("Ignore certificate errors") }}</label></template>
                </fieldset>
                <fieldset class="grid gap-3 border-t pt-4" style="border-color: var(--app-border)"><legend class="font-semibold">{{ $t("Authentication") }}</legend>
                    ...
                    <template v-if="draft.connection.authentication.credentials.enabled"><label class="flex flex-col gap-1">{{ $t("Username") }} <InputText v-model="draft.connection.authentication.credentials.username" required /><span class="muted text-sm">{{ $t("Username for authentication") }}</span></label><label class="flex flex-col gap-1">{{ $t("Password") }} <Password v-model="draft.connection.authentication.credentials.password" :feedback="false" toggle-mask /><span class="muted text-sm">{{ $t("Password for authentication") }}</span></label></template>
                    ...
                    <template v-if="draft.connection.authentication.clientCertificate.enabled"><label class="flex flex-col gap-1">{{ $t("Certificate") }} <Textarea v-model="draft.connection.authentication.clientCertificate.certificate" rows="3" required /><span class="muted text-sm">{{ $t("The full certificate as a multi-line string") }}</span></label><label class="flex flex-col gap-1">{{ $t("Key") }} <Textarea v-model="draft.connection.authentication.clientCertificate.key" rows="3" required /><span class="muted text-sm">{{ $t("The full key as a multi-line string") }}</span></label></template>
                </fieldset>
                ...
                <fieldset class="grid gap-3 border-t pt-4" style="border-color: var(--app-border)"><legend class="font-semibold">{{ $t("Customizations") }}</legend>
                    <label class="flex flex-col gap-1">{{ $t("Topic prefix") }} <InputText v-model="draft.customizations.topicPrefix" :placeholder="properties.data.value?.defaults.customizations.topicPrefix" @input="normalizePrefix" /><span class="muted text-sm">{{ $t("MQTT topic prefix") }}</span></label>
                    <label class="flex flex-col gap-1">{{ $t("Identifier") }} <InputText v-model="draft.identity.identifier" :placeholder="properties.data.value?.defaults.identity.identifier" @input="normalizeIdentifier" /><span class="muted text-sm">{{ $t("The machine-readable name of the robot") }}</span></label>
