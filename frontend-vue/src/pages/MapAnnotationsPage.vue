...
import {Capability, ValetudoMapAnnotationType, type Point} from "../../../frontend/src/api/types";
import {RawMapEntityType, type RawMapEntity} from "../../../frontend/src/api/RawMapData";
import {fetchMapAnnotationsProperties, sendMapAnnotationsUpdate} from "../../../frontend/src/api/client";
...
<template>
    <section class="panel">
        <h1 class="mb-4 text-2xl font-bold">Map annotations</h1>
        <Message v-if="!capabilities.includes(Capability.MapAnnotations)" severity="warn">Annotations are unavailable on this robot.</Message>
        <p v-else-if="map.isPending.value || properties.isPending.value" role="status">Loading map…</p>
        <Message v-else-if="map.isError.value || properties.isError.value" severity="error">Unable to load map or annotation types.</Message>
        <template v-else-if="displayedMap">
            <Message v-if="!canEdit" severity="info" class="mb-3">Dock the robot to edit annotations.</Message>
            <div class="mb-3 flex flex-wrap gap-2"><Button label="Pan" :outlined="mode !== 'pan'" @click="mode = 'pan'" /><Button v-for="type in properties.data.value?.supportedAnnotationTypes" :key="type" :label="`Add ${type}`" :disabled="!canEdit" :outlined="mode !== type" @click="mode = type" /></div>
            <p class="muted mb-2 text-sm">{{ mode === 'pan' ? 'Drag an existing annotation to move it; drag elsewhere or pinch to move the map.' : 'Drag on the map to draw the annotation.' }}</p>
            ...
            <div v-for="(entity, index) in draft" :key="index" class="border-b py-3" style="border-color: var(--app-border)"><div class="mb-2 flex items-center justify-between"><strong>{{ entity.type }}</strong><Button label="Remove" text severity="danger" :disabled="!canEdit" @click="draft.splice(index, 1)" /></div><div class="flex flex-wrap gap-2"><label v-for="(coordinate, pointIndex) in entity.points" :key="pointIndex" class="flex items-center gap-1 text-sm">{{ pointIndex % 2 ? 'Y' : 'X' }}{{ Math.floor(pointIndex / 2) + 1 }} <InputNumber :model-value="coordinate" :disabled="!canEdit" :min="0" :max="pointIndex % 2 ? maxY : maxX" :use-grouping="false" input-class="w-24" @update:model-value="setCoordinate(index, pointIndex, $event)" /></label></div></div>
            ...
            <Message v-if="save.isError.value" severity="error" class="mt-3">Unable to save annotations.</Message>
        </template>
    </section>
</template>
