...
<template>
    <section class="panel max-w-3xl">
        <h1 class="mb-1 text-2xl font-bold">Valetudo options</h1>
        <p class="muted mb-5">Tunables and actions provided by Valetudo</p>
        <div class="space-y-5">
            <ValetudoActivation v-if="aprilFools" />
            <RouterLink class="nav-card" to="/options/valetudo/analytics">Analytics</RouterLink>
            <div class="flex flex-wrap items-center justify-between gap-3 border-b pb-4" style="border-color: var(--app-border)"><div><h2 class="font-semibold">Custom friendly name</h2><p class="muted text-sm">{{ customizations.data.value?.friendlyName || 'Set a name for network advertisement and MQTT' }}</p></div><Button label="Edit" outlined :disabled="customizations.isPending.value" @click="editing = true" /></div>
            <div class="flex flex-wrap items-center justify-between gap-3 border-b pb-4" style="border-color: var(--app-border)"><div><h2 class="font-semibold">Update channel</h2><p class="muted text-sm">Select the channel used by the updater</p></div><Select :model-value="updater.data.value?.updateProvider" :options="[{label: 'Release', value: 'github'}, {label: 'Nightly', value: 'github_nightly'}]" option-label="label" option-value="value" :disabled="updater.isPending.value || saveUpdater.isPending.value" @update:model-value="value => saveUpdater.mutate({updateProvider: value})" /></div>
            <label v-if="capabilities.includes(Capability.Duststreaming)" class="flex items-center gap-3 border-b pb-4" style="border-color: var(--app-border)"><Checkbox :model-value="duststream.data.value?.enabled ?? false" binary :disabled="duststream.isPending.value || saveDuststream.isPending.value" @update:model-value="value => saveDuststream.mutate({enabled: Boolean(value)})" /> Enable camera stream</label>
            <div class="flex flex-wrap items-center justify-between gap-3"><div><h2 class="font-semibold">Restore default configuration</h2><p class="muted text-sm">Only Valetudo settings are affected</p></div><Button label="Restore" severity="danger" outlined @click="confirmRestore = true" /></div>
        </div>
        <Message v-if="customizations.isError.value || updater.isError.value || duststream.isError.value || saveName.isError.value || saveUpdater.isError.value || saveDuststream.isError.value || restore.isError.value" severity="error" class="mt-4">A settings request failed.</Message>
        <Dialog v-model:visible="editing" modal header="Custom friendly name" class="max-w-md"><p class="mb-3">Up to 24 letters, digits, spaces or hyphens.</p><InputText v-model="friendlyName" maxlength="24" class="w-full" @input="normalizeName" /><div class="mt-5 flex justify-end gap-2"><Button label="Cancel" text @click="editing = false" /><Button label="Save" :loading="saveName.isPending.value" @click="normalizeName(); saveName.mutate({friendlyName})" /></div></Dialog>
        <Dialog v-model:visible="confirmRestore" modal header="Restore default Valetudo configuration?" class="max-w-md"><p>This will not affect Wi-Fi settings or map data.</p><div class="mt-5 flex justify-end gap-2"><Button label="Cancel" text @click="confirmRestore = false" /><Button label="Restore" severity="danger" :loading="restore.isPending.value" @click="restore.mutate()" /></div></Dialog>
    </section>
</template>
